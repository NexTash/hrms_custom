...
class AccountsAndPayrollPayableReport(ReceivablePayableReport):
	...
	def get_data(self):
		...
		if self.include_payroll_payable:
			...
			if payroll_data:
				# Insert payroll data before any totals/summary rows
				...
				# Insert payroll entries at the determined position
				for j, entry in enumerate(payroll_data):
					self.data.insert(insert_index + j, entry)
					
					# Add subtotal row for this payroll entry if grouping is enabled
					if self.filters.get("group_by_party"):
						subtotal_row = self.create_subtotal_row(entry)
						self.data.insert(insert_index + j + 1, subtotal_row)
